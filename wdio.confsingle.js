...
exports.config = {
    ...
    capabilities: {
        ...
        driver: {
            port: 4723,
            desiredCapabilities: {
                app: "/Users/kwilliamson/Downloads/build-x86_64-local-fbobjc-wilde.132287010.ipa",
                platformName: "iOS",
    ...
    baseUrl: 'https://m.intern.facebook.com/mobile_builds/bisect/detail_new/?bisect_fbid=1348028665336090',
    //
...
//        const bisectLink = ('https://m.intern.facebook.com/mobile_builds/bisect/detail_new/?bisect_fbid=' + bisectNumber)
        ...
        chromeBrowser.addCommand("grabBisectBuild", function(customVar) {
            chromeBrowser.url(bisectLink)
             appNumber = chromeBrowser.getAttribute("/html//div[@id='root']/div[3]/div[@class='right']/a[@role='button']", 'rel')
             fileName = ("build-x86_64-local-fbobjc-wilde." + appNumber + ".ipa")
             filePath = (global.downloadDir + "/" + fileName)
                chromeBrowser.click("/html//div[@id='root']/div[3]/div[@class='right']/a[@role='button']")//    
                chromeBrowser.call(function () {
                    return checkExistsWithTimeout(filePath, 600000)
                });
            })
        ...
        chromeBrowser.addCommand("bisectToolLogin", function(customVar) {
            chromeBrowser.url(bisectLink)
            chromeBrowser.$('#m_login_email').setValue(input.username)
            chromeBrowser.$('#m_login_password').setValue(input.password)
            chromeBrowser.click('#u_0_5')
            chromeBrowser.pause(10000)
        })
       ...
       driver.addCommand("fbUrl", function(customVar) {
            ...
            chromeBrowser.addCommand("checkIfDone", function(customVar) {
            if (chromeBrowser.$("//div[@id='root']//span[@class='textHighlight']").getText() === "Congratulations! You finished the bisect") {
                console.log(chromeBrowser.$("//div[@id='root']//span[@class='textHighlight']").getText())
                step = 30
            }})

            chromeBrowser.addCommand("logStepNum", function(customVar) {
            var stepNum;
            for (stepNum = 0; stepNum < 25; stepNum++) {
            if (chromeBrowser.isExisting("/html//div[@id='root']//strong[.=' Estimated steps left: " + stepNum + "']")) {
                console.log(chromeBrowser.$("/html//div[@id='root']//strong[.=' Estimated steps left: " + stepNum + "']").getText())
            }
            else { step = 26 } }});
